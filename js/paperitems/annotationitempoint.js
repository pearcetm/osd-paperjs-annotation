class AnnotationItemPoint{
    constructor(geoJSON, allowExportToGeoJSON = true){
        ...
        if(!props.subtype){
            point = subtypeNone(geoJSON)
        } else if(props.subtype == 'Rectangle') {
            console.log('Point:Rectangle!');
            point = subtypeRectangle(geoJSON);
        }

        if(!allowExportToGeoJSON){
            delete(point.toGeoJSONGeometry);
        }
        return point;
    }
}
...
function subtypeRectangle(geoJSON){
    let center = new paper.Point(geoJSON.geometry.coordinates.slice(0,2));
    let width = geoJSON.geometry.properties.width;
    let height = geoJSON.geometry.properties.height;

    let topLeft = center.subtract(new paper.Point(width/2, height/2));
    let size = new paper.Size(width, height);
    let rect = new paper.Path.Rectangle(topLeft, size);

    rect.toGeoJSONGeometry = function () {
        let g = point.config.geometry;
        g.coordinates = [rect.bounds.center.x, rect.bounds.center.y];
        return g;
    };
