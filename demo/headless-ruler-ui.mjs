...
export function setupRulerHeadlessButtonsAndOutlet(toolkit, options = {}) {
    const headlessControlsId = options.headlessControlsId ?? 'ruler-headless-controls';
    const toolOutletId = options.toolOutletId ?? 'ruler-headless-tool-outlet';
    const headlessControls = document.getElementById(headlessControlsId);
    const buttonContainer = headlessControls?.querySelector('.headless-tool-buttons');
    const outlet = document.getElementById(toolOutletId);
    ...
    function bringToolkitOverlayForward() {
