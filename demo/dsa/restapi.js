...
function REST(prefix){
    ...
    axios.interceptors.response.use((response) => {
        ...
        if(error.config.retry){
            ...
            if(retryAfter > 0) return setTimeout(()=>axios.request(error.config), retryAfter);
        }
    ...
    const api = function(type,endpoint,options={}){
       ...
       let  axParams = {
        ...
        if(options.parse==false) axParams.transformResponse=res=>{ return res; };
    ...
    api.setapi_key = function(apikey){
        let retrier = (()=>{
            let retryCounter=1;
            return (response)=>{console.log('Retrying',response)
                if(response.status<500) return 0;
                if(retryCounter>10) return 0;
