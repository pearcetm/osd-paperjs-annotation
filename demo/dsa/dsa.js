//depends on restapi.js
function DigitalSlideArchiveAPI(baseurl='',apiurl='/api/v1'){
    var dsa = REST(baseurl+apiurl);
    dsa.LoginSystem = (function(){
        var loginScreen = $(loginHTML());
        this.getLoginScreen = function(){return loginScreen;}
        this.login = function(){
            var str = btoa($('#username').val() + ':' + $('#password').val());
            loginScreen.removeClass('login-error')
            return dsa.setbasicauth(str).get('user/authentication').then(onlogin).catch(d=>{
        ...
        function onlogin(d){
            ...
            loginScreen.find('#loginname').text(d.user.login).attr('href',baseurl+'#useraccount/'+d.user._id+'/info');
        ...
        function loginHTML(){
            ...
            <div id='dsa-login-screen'>
                 <div class='dsa-intro'>Built using the <a href='${baseurl}' target='_blank'>Digital Slide Archive</a> engine.</div>
